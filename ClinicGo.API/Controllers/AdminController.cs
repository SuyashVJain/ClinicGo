...
public class AdminController : ControllerBase
{
    ...
    // IST = UTC+5:30. DateTime.Today uses the server's local clock (UTC on Linux),
    // so we always derive "today" from UTC by converting to IST explicitly.
    ...
    public async Task<IActionResult> Compute()
    {
        ...
        // 1. Daily revenue — convert stored UTC timestamp to IST before comparing
        var dailyRevenue = await _db.Payments
            .Where(p => p.PaidAt.HasValue &&
                        DateOnly.FromDateTime(
                            TimeZoneInfo.ConvertTimeFromUtc(p.PaidAt.Value, IST)) == today &&
                        p.Status == "PAID")
            .SumAsync(p => p.Amount);
        ...
        // 5. Daily summary — aggregated KPIs read by the dashboard
        var totalPatients = await _db.Users
            .CountAsync(u => u.Role == "PATIENT" && u.Status == "ACTIVE");
        var apptToday = await _db.Appointments
            .CountAsync(a => a.AppointmentDate == today);
        var activeDoctorCount = await _db.Doctors
            .CountAsync(d => d.IsAvailable && d.User.Status == "ACTIVE");

        await UpsertSnapshot("DAILY_SUMMARY", today,
