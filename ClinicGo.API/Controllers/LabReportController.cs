...
public class LabReportController : ControllerBase
{
    private static readonly List<object> _reports = new()
    {
        new { ReportId=1, PatientId=5, FileName="BloodTest_April2026.pdf", FileType="PDF", UploadedAt="2026-04-20T10:00:00Z" },
        new { ReportId=2, PatientId=5, FileName="XRay_Chest_2026.jpg",     FileType="JPG", UploadedAt="2026-04-22T14:30:00Z" },
        new { ReportId=3, PatientId=6, FileName="LFT_Report.pdf",          FileType="PDF", UploadedAt="2026-04-18T09:00:00Z" },
        new { ReportId=4, PatientId=7, FileName="CBC_Complete.pdf",        FileType="PDF", UploadedAt="2026-04-25T11:00:00Z" },
    };

    [HttpPost("upload")]
    [Authorize(Roles = "PATIENT")]
    public IActionResult Upload(IFormFile? file)
        => Ok(new { ReportId=5, FileName="uploaded_report.pdf", FileType="PDF", UploadedAt=DateTime.UtcNow, message="Lab report uploaded successfully." });

    [HttpGet("patient/{patientId}")]
    public IActionResult GetByPatient(int patientId)
    {
        var list = _reports.Where(r => ((dynamic)r).PatientId == patientId).ToList();
        return Ok(list);
    }

    [HttpGet("{reportId}/download")]
    public IActionResult Download(int reportId)
        => Ok(new { message="File download would stream here in production.", reportId });
}